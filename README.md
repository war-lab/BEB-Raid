...
2. `packages/api/.dev.vars.example` を参考に `packages/api/.dev.vars` を作成し（招待コード `INVITE_CODE` と管理用の `ADMIN_TOKEN` を記載。gitignore 対象）、共有APIをローカル起動する。
...
| `packages/shared-schema` | 問題パックスキーマ・共有API契約型の単一正本（app / cli / api / review-ui が import） |
...
| 25 | `25_ビジュアル刷新計画_M4以降の未適用画面.md` | 20 の続編。M4 で新設されたイベントバトル・ゴースト画面と 20 の適用漏れの棚卸し・ビジュアル方向性（V-9〜V-23・判断 JV-4〜JV-11） |
| 26 | `26_ビジュアル改修_レイアウトとブランド整合.md` | 25 完了後の実機確認で見つかった、装飾では直らないレイアウト・ブランド整合の穴の棚卸し（A-1〜A-8） |
| 27 | `27_出題と回答のUXストレス点検.md` | 出題・回答フローの負荷点検の所見書（所見 S-1〜S-38・判断 J-89〜J-97） |
| 28 | `28_改修計画_出題と回答のUXストレス.md` | 27 を実行に落とす自走タスクシート（T-155〜T-178・判断 J-89〜J-99・人間タスク H-16） |
| 29 | `29_全量レビュー棚卸し.md` | アプリ・共有API・コンテンツ・ドキュメントの全量レビュー所見書（Q-1〜Q-113・判断 J-100〜J-114） |
| 30 | `30_改修計画_全量レビュー棚卸し.md` | 29 を実行に落とす自走タスクシート（T-179〜T-264・9 フェーズ） |
| 31 | `31_敵対的検証棚卸し.md` | 敵対的検証の所見書（K-1〜K-99・判断 J-115〜J-126）。29 が検出できなかったものを 9 観点で探した結果 |
| 32 | `32_改修計画_敵対的検証.md` | 31 を実行に落とす自走タスクシート（T-274〜T-365 の 92 タスク・10 ウェーブ）。人手レビューの代替工程の定義（8 節）を含む |
...
読解パート（docs 24）はフェーズ R-1（Part6・Part7 単一）が完了し配信中です。R-2（Part7 複数パッセージ、T-143〜T-145）は 2026-07-31 に実装が完了していますが、配信は人手レビュー（H-R1）待ちで止まっています。ADR 0006 の判断 5 により人手レビューを経ないコンテンツは配信できないため、生成器・在庫・画面が完成していても利用者にはまだ 1 問も届いていません（在庫は目標 15 セットに対し 5 セット）。
...
全量レビュー（docs 29・30）と敵対的検証（docs 31・32）による改修は、2026-08-12 に全ウェーブを dev へ統合しました。ただしその後の文書監査で、数値の完了条件を持つ 7 タスクが未達のまま完了扱いになっていたことが判明しています。特にコンテンツのレビュー工程（T-355）は、記録が無いまま配信されているパックが 1 件あり、`CLAUDE.md` の不変条件に対する逸脱です。詳細と是正方針は `docs/STATUS.md` 1.4 節を参照してください。

**dev から main へのマージは保留中です。** GitHub Pages の公開 URL は main への push でのみ更新されるため、公開 URL には上記の改修がまだ反映されていません。
