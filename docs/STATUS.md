...
- **検証**: ルート `npm run lint`・`npm run format:check`・`npm run build`・`npm test`（api 125・app 888・cli 338・review-ui 15・shared-schema 96）が全通過。新規テスト `HostProjectionLayout.test.tsx` 6件、`BattleHostScreen.test.tsx` に投影用意匠3件を追加。既存の `BattleHostScreen.test.tsx`・`StandingsList.test.tsx`・`BattleAward.test.tsx`・`BattleScreen.test.tsx` は無修正で通過した。
