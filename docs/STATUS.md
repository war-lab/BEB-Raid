...
**M1 実装中 — F3・F4 完了。F5（コンテンツパイプライン）着手: T-30完了。T-25以降はLLM実行コスト・B-1/B-2判断が絡むため人間確認待ち。** タスク進捗: 25 / 38 完了。
...
| F5 コンテンツパイプライン（T-24〜T-34) | 🔶 一部完了 | T-24・T-30 完了。T-25以降はLLM実行コストが絡むため人間の実行判断待ち（B-1/B-2は未解決だがT-25はLLM生成全振りで進行可）。**M1 全体の律速** |
...
2. **F5 前半** — T-30 ✅完了。**次は T-25（自前頻出度リスト構築）**: `@anthropic-ai/sdk`での実LLM呼び出し（API課金発生）を伴うため、コマンド実装・ロジックまでは自走可能だが、**実際にAPIキーを使った生成実行は人間の承認を得てから行う**（🟡人間チェックポイント。ANTHROPIC_API_KEY環境変数の用意も必要）。T-26/27/28はさらにT-30完了（済）＋T-25完了が前提で、200語/50問規模の生成実行はT-26/27/28シート内で明示的に🔴人間チェックポイント
