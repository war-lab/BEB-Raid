...
1. **B-2 の解消（TTS 調達判断）** — 人間の判断が必要。T-31 とその下流（T-32実コンテンツ・T-37）が止まっている
2. **F4: 学習モードUI** — S-A1 縦切りスライス（T-15 → T-16 → T-17）から。エンジンは C-4 の型（`engine/types.ts`）経由で利用する
3. **F5 前半（並行可）** — T-30 → T-25 → T-26/27/28 のコマンド実装。生成実行と目視レビューは人間の稼働待ち（M1の律速）
