...
## 2026-07-29: devのCI（format:check）が赤だったのを解消（ブランチ `task/fix-ci-prettier-release-note`。dev起点）

**dev のCIが 2026-07-29 3:48 のH-13マージ以降ずっと失敗していた**（`gh run list` で確認。H-13以前の dev は success）。原因は `npm run format:check`（`prettier --check .`）が `BEB-Raid-リリースノート.html` を未整形と報告することで、H-13でこのファイルを追跡対象に入れたときから継続していた。

- H-13の記録には「既知。data URIを含む700KB超のファイルをPrettierで整形すると差分がノイズになるため触っていない」とあるが、**CIが赤のままになる副作用が見落とされていた**。以降のすべてのPRが `mergeStateStatus: UNSTABLE` になり、CIの緑を根拠にできない状態だった。
- 対処は `.prettierignore` に `BEB-Raid-*.html` を追加すること。**整形はしない**（H-13の判断＝差分ノイズを避けるを維持する）。スクリーンショットをWebPのdata URIで埋め込んだ配布用の生成物で、手で編集する正本ではないため、既存の `docs/design/`（凍結された参考資料）・`content/`（CLI生成物）と同じ扱いにする。ワイルドカードにしたのは同種の配布HTML（取説）が今後増えても同じ穴を踏まないため。
- **検証**: `npx prettier --check .` が `All matched files use Prettier code style!` になることを確認した。
