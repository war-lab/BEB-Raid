...
**最終更新: 2026-07-13**（更新ルール: [09_開発体制](09_開発体制.md) 7節。タスクの着手・完了・ブロッカー変化のたびに同じPRで更新する）

## 今どこにいるか（1行）

**M1 実装中 — F3・F4 完了。F5前半（T-24〜T-31）ほぼ完了: T-24〜T-31すべて完了（T-31は完了条件(a)まで）。T-25〜T-28はAIクロスレビュー実施済み（major9件/minor41件検出、major全件+minor30件修正）。次は全量TTS生成（🟡人間の聴感確認込み）→T-32（パックビルド）。** タスク進捗: 31 / 38 完了。
...
| F5 コンテンツパイプライン（T-24〜T-34) | 🔶 一部完了 | T-24〜T-31（T-31は完了条件(a)まで）完了。テキスト生成コンテンツはAIクロスレビュー実施済み（下記※レビュー欄）。次は全量TTS生成→T-32。**M1 全体の律速** |
...
| B-2 | TTSの調達（Azure Speech の個人利用可否とコスト実測） | ✅ **解消（2026-07-13）**: **Piper（ローカルTTS、MITライセンス）採用に決定**。発起人の最優先事項が「無料の徹底」（利用規模10人前後）だったため、クラウドTTS（Azure F0=月50万文字無料／Google=月100〜400万文字無料）はいずれも無料枠内でも課金アカウント登録が必要な点をリスクとみなし、アカウント登録・カード登録・クラウド課金の仕組み自体が存在しないPiperを選定。ローカルビルド時生成（05のネイティブ化・ランタイム課金ゼロ方針）とも合致。**未検証事項**: en_AU（豪アクセント）ボイスの有無（T-31実装時に確認。無い場合は話者ローテーションを米/英2アクセントに縮退する可能性あり） |
...
4. **T-25〜T-31（T-31は完了条件(a)まで）全て完了** — テキストコンテンツ（頻出度リスト・語彙カード・Part2・Part5・key単語類題）はAIクロスレビューも実施済み。**次は全量TTS生成**: `content/drafts/*.jsonl`の全件（語彙200＋Part2/Part5各50＋類題57＝計357件）をPiperで実際に音声化し、🟡人間の聴感確認を経てからT-32（パックビルド）に進む
