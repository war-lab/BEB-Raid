...
**M1 実装中 — F3・F4 完了。F5着手: T-30・T-25完了（T-25はランタイムAPI呼び出しをせず直接データ構築。🟡人間目視確認は未実施）。次はT-26〜T-28（本番生成実行はAPIコスト発生のため人間承認要）。** タスク進捗: 26 / 38 完了。
...
| F5 コンテンツパイプライン（T-24〜T-34) | 🔶 一部完了 | T-24・T-25・T-30 完了。T-26以降は実際のLLM生成実行（APIコスト発生）が絡むため人間の実行判断待ち。**M1 全体の律速** |
...
| T-25 | 自前頻出度リスト構築 | ✅ 完了（コーパス条件はN/A。🟡 S200語の人間目視確認は未実施） | 2026-07-10 |
...
2. **F5 前半** — T-30・T-25 ✅完了（T-25はユーザー指示によりAPI呼び出しなしでエージェントが直接データ構築。🟡S200語の人間目視確認は未実施のまま残っている）。**次は T-26〜T-28（語彙カード/Part2/Part5生成）**: T-25で確立した「ランタイムAPIを呼ばずエージェントが直接コンテンツを書く」方針を踏襲するか、実際に`@anthropic-ai/sdk`でAPIを呼ぶ元の設計に戻すかは要判断（200語分の語彙カード・50問×2の問題文生成は分量が多く、エージェント直接執筆だと1タスクの作業量が非常に大きくなる）。いずれの方針でも、生成物のバリデーション通過確認とレビュー（T-30の`review-export`/`review-import`経由）は必須
