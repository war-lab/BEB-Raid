...
| データ | フィールド | 備考 |
|--------|-----------|------|
| members | deviceToken, displayName, ratingBand(粗い帯のみ), dailyGoal(自己申告), emaDailyDamage(週次補正値) | 本名・詳細成績は持たない。dailyGoal/emaDailyDamageはボスHP算出の入力（J-48。docs/17 3.4節） |
| raids | bossId, profileJson, status, startAt, endAt | |
| damageLogs | bossId, deviceToken, damage, questionCount, answeredAt, syncedAt | **ダメージ換算値のみ。正誤詳細は送らない**。answeredAtは帰属判定（週境界跨ぎ。J-49）に使う受信専用の時刻でクライアント時計は信用しない（サーバー側クランプ。docs/17 3.4節） |
