...
- `format: text_passage`（Part6/7・T-103）は `passages[]`（刺激文書 `{ id, kind, text }`）＋ `subQuestions[]` で構成する（audio_set と同じ「1刺激＋設問複数」構造。刺激が音声でなく本文になる）。Part6は本文に空所マーカー `[[1]]…[[n]]` を埋め、n番目の設問が `[[n]]` に対応する。Part7単一は passages 1件、Part7複数パッセージは passages 2〜3件。相互参照設問は `subQuestions[].tags`（任意）で設問単位のタグ付けができる（弱点集計の粒度用）。詳細は [ADR 0006](adr/0006-読解パートの完成方針.md)・[docs/18](18_読解パート実装計画.md) 3.1節。
