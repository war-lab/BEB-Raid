...
| settings | イヤホンなしモード, 再生速度既定, BYOKキー(端末内平文保存。支出上限付きキーを推奨する注記を表示), キャッシュ済みパック一覧 | |

- 容量目安: attempts 年3万行×200B ≈ 6MB。音声キャッシュ（Cache Storage側）が支配的で100–200MB。
- **エクスポート/インポート**: 全ストアをJSON1ファイルに書き出し/読み込み。機種変とiOSストレージ退避（01のR-5）への保険。共有API接続時は週次自動バックアップも可（v2）。
...
- TTSは米/英/豪/加の話者をローテーションし `audioMeta.accent` に記録。Part2は設問と応答で別話者。
