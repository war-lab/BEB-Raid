...
| データ | フィールド | 備考 |
|--------|-----------|------|
| members | deviceToken, displayName, dailyGoal(自己申告), emaDailyDamage(週次補正値), registeredAt, ratingBand(粗い帯のみ。**M3未実装・M4予定**) | 本名・詳細成績は持たない。dailyGoal/emaDailyDamageはボスHP算出の入力（J-48。docs/17 3.4節）。registeredAtは初回登録時刻（再登録では引き継ぐ） |
