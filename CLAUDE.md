...
| `docs/18_読解パート実装計画.md` | 読解（Part6/7）完成の自走タスクシート（T-103〜T-110・判断J-51〜J-53・人間タスクH-R1/H-R2）。**読解実装着手時はまずここを読む**。ADR 0006・02・03に従属。**注意**: 上記の別系統の18番文書（改修計画_表示更新とUX残課題）と番号・タスクID（T-103〜）が衝突している。dev合流時（2026-07-21）に発覚。要リナンバリング（別途対応） |
| `docs/20_ビジュアル刷新計画.md` | 脱ワイヤーフレームのビジュアル刷新（診断・方針・V-1〜V-8・判断JV-1〜JV-3）。**ビジュアル刷新着手時はまずここを読む**。07に従属 |
| `docs/21_M4タスク分解.md` | M4（ゴーストレイド＋昼イベント）のタスク分解（T-123〜T-131・判断J-62〜J-70・人間タスクH-4〜H-6）。01〜07が正本で本書は従属 |
| `docs/22_M4実装計画.md` | M4の自走タスクシート（事前決定事項・WebSocketプロトコル・実行順序）。**M4実装着手時はまずここを読む**。21に従属 |
| `docs/23_M5ネイティブ化計画.md` | M5のうちCapacitorネイティブ化と通知手段の計画（T-132〜T-137・T-146〜T-149・判断J-71〜J-78・J-82〜J-86・人間タスクH-7〜H-11）。分解と自走シートを1冊に統合。**ネイティブ化の着手は保留中**で、先行してiOS PWAのWeb Push検証（T-149）を行う（ADR 0007）。**アプリ化・通知の着手時はまずここを読む**。05の7節に従属 |
