 ...
 *
 * T-313（K-44）: この値そのものが選択結果に占める混入率ではない。mixCountは
 * appropriateの件数に対する2割で、それをappropriate（分母）へ追加混入するため、
 * 選択結果内の実際の混入率は mixCount/(appropriate.length+mixCount) ≈
 * HARD_MIX_RATIO/(1+HARD_MIX_RATIO) ≈ 16.7%（名目の20%より薄まる）。
 * 20問選択なら実測で平均3問程度（「20問選択で高難度4問」という従来の記述は
 * 名目値20%から単純計算した値で、実挙動とずれていた）。
 * 「たまに歯が立たない問題が来る」程度に収まり、かつプール後半へ到達する経路を
 * 確実に作れる。実機の体感で調整する前提の値
 */
